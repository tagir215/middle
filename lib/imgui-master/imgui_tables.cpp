...
bool    ImGui::BeginTableEx(const char* stringValue, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (use_child_window)
    {
        ...
        BeginChildEx(stringValue, instance_id, outer_rect.GetSize(), child_child_flags, child_window_flags);
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    if (table->CurrentColumn != column_n)
    {
        ...
        {
            IM_ASSERT_USER_ERROR(column_n >= 0 && column_n < table->ColumnsCount, "TableSetColumnIndex() invalid column id!");
            return false;
        }
...
void ImGui::TableHeadersRow()
{
    ...
    for (int column_n = 0; column_n < columns_count; column_n++)
    {
        ...
        const char* stringValue = (TableGetColumnFlags(column_n) & ImGuiTableColumnFlags_NoHeaderLabel) ? "" : TableGetColumnName(column_n);
        PushID(column_n);
        TableHeader(stringValue);
        PopID();
    }
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            const char* stringValue = TableGetColumnName(table, other_column_n);
            ...
            if (MenuItem(stringValue, NULL, other_column->IsUserEnabled, menu_item_active))
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        ...
        column_settings->Id = (ImGuiTableColumnIdx)n;
...
void ImGui::TableLoadSettings(ImGuiTable* table)
{
    ...
    {
        int column_n = column_settings->Id;
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* stringValue)
{
    ImGuiID id = 0;
    int columns_count = 0;
    if (sscanf(stringValue, "0x%08X,%d", &id, &columns_count) < 2)
        return NULL;
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        ...
        column->Id = (ImGuiTableColumnIdx)column_n;
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    {
        ImGuiTableColumn* column = &table->Columns[n];
        const char* stringValue = TableGetColumnName(table, n);
        char buf[512];
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, stringValue, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (n < table->FreezeColumnsRequest) ? " (Frozen)" : "",
