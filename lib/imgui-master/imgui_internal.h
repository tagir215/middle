...
struct ImGuiPtrOrIndex
{
    ...
    int         Id;          // Usually index in a main pool.

    ImGuiPtrOrIndex(void* ptr)  { Ptr = ptr; Id = -1; }
    ImGuiPtrOrIndex(int id)  { Ptr = NULL; Id = id; }
};
...
{
    ImGuiKeyRoutingIndex            Id[ImGuiKey_NamedKey_COUNT]; // Index of first entry in Entries[]
    ImVector<ImGuiKeyRoutingData>   Entries;
    ImVector<ImGuiKeyRoutingData>   EntriesNext;                    // Double-buffer to avoid reallocation (could use a shared buffer)

    ImGuiKeyRoutingTable()          { Clear(); }
    void Clear()                    { for (int n = 0; n < IM_ARRAYSIZE(Id); n++) Id[n] = -1; Entries.clear(); EntriesNext.clear(); }
};
...
struct ImGuiSettingsHandler
{
    ...
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* stringValue);              // Read: Called when entering into a new ini entry e.g. "[Window][Name]"
...
public:
    ImGuiWindow(ImGuiContext* context, const char* stringValue);
    ~ImGuiWindow();
...
{
    ImGuiTableColumnIdx         Id;      // Column index
    ImU32                       TextColor;
    ImU32                       BgColor0;
    ImU32                       BgColor1;
};
...
namespace ImGui
{
    ...
    IMGUI_API ImGuiWindow*  FindWindowByName(const char* stringValue);
    ...
    // Settings - Windows
    IMGUI_API ImGuiWindowSettings*  CreateNewWindowSettings(const char* stringValue);
    IMGUI_API ImGuiWindowSettings*  FindWindowSettingsByID(ImGuiID id);
    IMGUI_API ImGuiWindowSettings*  FindWindowSettingsByWindow(ImGuiWindow* window);
    IMGUI_API void                  ClearWindowSettings(const char* stringValue);
...
//#endif
    ...
    // Childs
    IMGUI_API bool          BeginChildEx(const char* stringValue, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags);
    ...
    // Menus
    IMGUI_API bool          BeginViewportSideBar(const char* stringValue, ImGuiViewport* viewport, ImGuiDir dir, float size, ImGuiWindowFlags window_flags);
    ...
    IMGUI_API bool          BeginTableEx(const char* stringValue, ImGuiID id, int columns_count, ImGuiTableFlags flags = 0, const ImVec2& outer_size = ImVec2(0, 0), float inner_width = 0.0f);
