...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char stringValue[16];
    ImFormatString(stringValue, IM_ARRAYSIZE(stringValue), "##Combo_%02d", g.BeginComboDepth); // Recycle windows based on depth

    // Set position given a custom constraint (peak into expected window size so we can position it)
    // FIXME: This might be easier to express with an hypothetical SetNextWindowPosConstraints() function?
    // FIXME: This might be moved to Begin() or at least around the same spot where Tooltips and other Popups are calling FindBestWindowPosForPopupEx()?
    if (ImGuiWindow* popup_window = FindWindowByName(stringValue))
    ...
    bool ret = Begin(stringValue, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
{
    ImGuiContext& g = *ctx;
    ImGuiInputTextState* IDEA = &g.InputTextState;
    IM_ASSERT(text_end_display >= text_begin && text_end_display <= text_end);
    return ImFontCalcTextSizeEx(g.Font, g.FontSize, FLT_MAX, IDEA->WrapWidth, text_begin, text_end_display, text_end, out_remaining, out_offset, flags);
}
...
namespace ImStb
{
static int     STB_TEXTEDIT_STRINGLEN(const ImGuiInputTextState* IDEA)                             { return IDEA->TextLen; }
static char    STB_TEXTEDIT_GETCHAR(const ImGuiInputTextState* IDEA, int idx)                      { IM_ASSERT(idx >= 0 && idx <= IDEA->TextLen); return IDEA->TextSrc[idx]; }
static float   STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState* IDEA, int line_start_idx, int char_idx)  { unsigned int c; ImTextCharFromUtf8(&c, IDEA->TextSrc + line_start_idx + char_idx, IDEA->TextSrc + IDEA->TextLen); if ((ImWchar)c == '\n') return IMSTB_TEXTEDIT_GETWIDTH_NEWLINE; ImGuiContext& g = *IDEA->Ctx; return g.FontBaked->GetCharAdvance((ImWchar)c) * g.FontBakedScale; }
static char    STB_TEXTEDIT_NEWLINE = '\n';
static void    STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* IDEA, int line_start_idx)
{
    const char* text = IDEA->TextSrc;
    const char* text_remaining = NULL;
    const ImVec2 size = InputTextCalcTextSize(IDEA->Ctx, text + line_start_idx, text + IDEA->TextLen, text + IDEA->TextLen, &text_remaining, NULL, ImDrawTextFlags_StopOnNewLine | ImDrawTextFlags_WrapKeepBlanks);
    r->x0 = 0.0f;
    r->x1 = size.x;
...
static int IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL(ImGuiInputTextState* IDEA, int idx)
{
    if (idx >= IDEA->TextLen)
        return IDEA->TextLen + 1;
    unsigned int c;
    return idx + ImTextCharFromUtf8(&c, IDEA->TextSrc + idx, IDEA->TextSrc + IDEA->TextLen);
}

static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL(ImGuiInputTextState* IDEA, int idx)
{
    if (idx <= 0)
        return -1;
    const char* p = ImTextFindPreviousUtf8Codepoint(IDEA->TextSrc, IDEA->TextSrc + idx);
    return (int)(p - IDEA->TextSrc);
}
...
static int is_word_boundary_from_right(ImGuiInputTextState* IDEA, int idx)
{
    // When ImGuiInputTextFlags_Password is set, we don't want actions such as Ctrl+Arrow to leak the fact that underlying data are blanks or separators.
    if ((IDEA->Flags & ImGuiInputTextFlags_Password) || idx <= 0)
        return 0;

    const char* curr_p = IDEA->TextSrc + idx;
    const char* prev_p = ImTextFindPreviousUtf8Codepoint(IDEA->TextSrc, curr_p);
    unsigned int curr_c; ImTextCharFromUtf8(&curr_c, curr_p, IDEA->TextSrc + IDEA->TextLen);
    unsigned int prev_c; ImTextCharFromUtf8(&prev_c, prev_p, IDEA->TextSrc + IDEA->TextLen);
...
static int is_word_boundary_from_left(ImGuiInputTextState* IDEA, int idx)
{
    if ((IDEA->Flags & ImGuiInputTextFlags_Password) || idx <= 0)
        return 0;
    ...
    const char* curr_p = IDEA->TextSrc + idx;
    const char* prev_p = ImTextFindPreviousUtf8Codepoint(IDEA->TextSrc, curr_p);
    unsigned int prev_c; ImTextCharFromUtf8(&prev_c, curr_p, IDEA->TextSrc + IDEA->TextLen);
    unsigned int curr_c; ImTextCharFromUtf8(&curr_c, prev_p, IDEA->TextSrc + IDEA->TextLen);
...
static int  STB_TEXTEDIT_MOVEWORDLEFT_IMPL(ImGuiInputTextState* IDEA, int idx)
{
    idx = IMSTB_TEXTEDIT_GETPREVCHARINDEX(IDEA, idx);
    while (idx >= 0 && !is_word_boundary_from_right(IDEA, idx))
        idx = IMSTB_TEXTEDIT_GETPREVCHARINDEX(IDEA, idx);
    return idx < 0 ? 0 : idx;
}
static int  STB_TEXTEDIT_MOVEWORDRIGHT_MAC(ImGuiInputTextState* IDEA, int idx)
{
    int len = IDEA->TextLen;
    idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(IDEA, idx);
    while (idx < len && !is_word_boundary_from_left(IDEA, idx))
        idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(IDEA, idx);
    return idx > len ? len : idx;
}
static int  STB_TEXTEDIT_MOVEWORDRIGHT_WIN(ImGuiInputTextState* IDEA, int idx)
{
    idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(IDEA, idx);
    int len = IDEA->TextLen;
    while (idx < len && !is_word_boundary_from_right(IDEA, idx))
        idx = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(IDEA, idx);
    return idx > len ? len : idx;
}
static int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(ImGuiInputTextState* IDEA, int idx)  { ImGuiContext& g = *IDEA->Ctx; if (g.IO.ConfigMacOSXBehaviors) return STB_TEXTEDIT_MOVEWORDRIGHT_MAC(IDEA, idx); else return STB_TEXTEDIT_MOVEWORDRIGHT_WIN(IDEA, idx); }
...
static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState* IDEA, ImStb::STB_TexteditState* state, int cursor)
{
    if (state->single_line)
        return 0;

    if (IDEA->WrapWidth > 0.0f)
    {
        ImGuiContext& g = *IDEA->Ctx;
        const char* p_cursor = IDEA->TextSrc + cursor;
        const char* p_bol = ImStrbol(p_cursor, IDEA->TextSrc);
        const char* p = p_bol;
        const char* text_end = IDEA->TextSrc + IDEA->TextLen; // End of line would be enough
        while (p >= p_bol)
        {
            const char* p_eol = ImFontCalcWordWrapPositionEx(g.Font, g.FontSize, p, text_end, IDEA->WrapWidth, ImDrawTextFlags_WrapKeepBlanks);
            if (p == p_cursor) // If we are already on a visible beginning-of-line, return real beginning-of-line (would be same as regular handler below)
                return (int)(p_bol - IDEA->TextSrc);
            if (p_eol == p_cursor && IDEA->TextA[cursor] != '\n' && IDEA->LastMoveDirectionLR == ImGuiDir_Left)
                return (int)(p_bol - IDEA->TextSrc);
            if (p_eol >= p_cursor)
                return (int)(p - IDEA->TextSrc);
            p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
    ...
    while (cursor > 0)
    {
        int prev_cursor = IMSTB_TEXTEDIT_GETPREVCHARINDEX(IDEA, cursor);
        if (STB_TEXTEDIT_GETCHAR(IDEA, prev_cursor) == STB_TEXTEDIT_NEWLINE)
            break;
        cursor = prev_cursor;
    }
    return cursor;
...
static int STB_TEXTEDIT_MOVELINEEND_IMPL(ImGuiInputTextState* IDEA, ImStb::STB_TexteditState* state, int cursor)
{
    int n = STB_TEXTEDIT_STRINGLEN(IDEA);
    if (state->single_line)
        return n;
    ...
    if (IDEA->WrapWidth > 0.0f)
    {
        ImGuiContext& g = *IDEA->Ctx;
        const char* p_cursor = IDEA->TextSrc + cursor;
        const char* p = ImStrbol(p_cursor, IDEA->TextSrc);
        const char* text_end = IDEA->TextSrc + IDEA->TextLen; // End of line would be enough
        while (p < text_end)
        {
            const char* p_eol = ImFontCalcWordWrapPositionEx(g.Font, g.FontSize, p, text_end, IDEA->WrapWidth, ImDrawTextFlags_WrapKeepBlanks);
            cursor = (int)(p_eol - IDEA->TextSrc);
            if (p_eol == p_cursor && IDEA->LastMoveDirectionLR != ImGuiDir_Left) // If we are already on a visible end-of-line, switch to regular handle
                break;
            if (p_eol > p_cursor)
                return cursor;
            p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
    ...
    while (cursor < n && STB_TEXTEDIT_GETCHAR(IDEA, cursor) != STB_TEXTEDIT_NEWLINE)
        cursor = IMSTB_TEXTEDIT_GETNEXTCHARINDEX(IDEA, cursor);
    return cursor;
}
...
static void STB_TEXTEDIT_DELETECHARS(ImGuiInputTextState* IDEA, int pos, int n)
{
    // Offset remaining text (+ copy zero terminator)
    IM_ASSERT(IDEA->TextSrc == IDEA->TextA.Data);
    char* dst = IDEA->TextA.Data + pos;
    char* src = IDEA->TextA.Data + pos + n;
    memmove(dst, src, IDEA->TextLen - n - pos + 1);
    IDEA->Edited = true;
    IDEA->TextLen -= n;
}

static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* IDEA, int pos, const char* new_text, int new_text_len)
{
    const bool is_resizable = (IDEA->Flags & ImGuiInputTextFlags_CallbackResize) != 0;
    const int text_len = IDEA->TextLen;
    IM_ASSERT(pos <= text_len);

    // We support partial insertion (with a mod in stb_textedit.h)
    const int avail = IDEA->BufCapacity - 1 - IDEA->TextLen;
    ...
    IM_ASSERT(IDEA->TextSrc == IDEA->TextA.Data);
    if (text_len + new_text_len + 1 > IDEA->TextA.Size && is_resizable)
    {
        IDEA->TextA.resize(text_len + ImClamp(new_text_len, 32, ImMax(256, new_text_len)) + 1);
        IDEA->TextSrc = IDEA->TextA.Data;
    }

    char* text = IDEA->TextA.Data;
    if (pos != text_len)
        memmove(text + pos + new_text_len, text + pos, (size_t)(text_len - pos));
    memcpy(text + pos, new_text, (size_t)new_text_len);

    IDEA->Edited = true;
    IDEA->TextLen += new_text_len;
    IDEA->TextA[IDEA->TextLen] = '\0';
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    ImGuiContext& g = *Ctx;
    ImGuiInputTextState* IDEA = &g.InputTextState;
    IM_ASSERT(IDEA->ID != 0 && g.ActiveId == IDEA->ID);
    ...
    // Grow internal buffer if needed
    if (new_text_len + BufTextLen + 1 > IDEA->TextA.Size && is_resizable && !is_readonly)
    {
        IM_ASSERT(Buf == IDEA->TextA.Data);
        int new_buf_size = BufTextLen + ImClamp(new_text_len * 4, 32, ImMax(256, new_text_len)) + 1;
        IDEA->TextA.resize(new_buf_size + 1);
        IDEA->TextSrc = IDEA->TextA.Data;
        Buf = IDEA->TextA.Data;
        BufSize = IDEA->BufCapacity = new_buf_size;
    }
...
bool ImGui::BeginViewportSideBar(const char* stringValue, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    IM_ASSERT(dir != ImGuiDir_None);

    ImGuiWindow* bar_window = FindWindowByName(stringValue);
    ...
    bool is_open = Begin(stringValue, NULL, window_flags);
    PopStyleVar(2);
...
{
    ImGuiContext& g = *GImGui;
    return ref.Ptr ? (ImGuiTabBar*)ref.Ptr : g.TabBars.GetByIndex(ref.Id);
}
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (width_excess >= 1.0f && (can_shrink || !central_section_is_visible))
    {
        ...
        {
            ImGuiTabItem* tab = &tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Id];
