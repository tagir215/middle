...
public:
	...
	void insertOperation(middle::GameState* gameState, int actionType, middle::Id intersectingId) {
		...
		if (actionType == bubbleInventoryItemType::NEW_ADDITION_TERM) {
			...
			action = std::make_shared<bubbleActions::NewAdditionTerm>(intersectingId, copyId, gameState->mouseState.mouseXZ_PlanePos);
		}
		else if (actionType == bubbleInventoryItemType::NEW_MULTIPLICATION_TERM) {
			middle::Id copyId = copyOfInsertItem(gameState);
			action = std::make_shared<bubbleActions::NewMultiplicationTerm>(intersectingId, copyId, gameState->mouseState.mouseXZ_PlanePos);
		}
		else if (actionType == bubbleInventoryItemType::NEW_POWER_TERM) {
			middle::Id copyId = copyOfInsertItem(gameState);
			action = std::make_shared<bubbleActions::NewPowerTerm>(intersectingId, copyId, gameState->mouseState.mouseXZ_PlanePos);
		}
		else if (actionType == bubbleInventoryItemType::INSERT_X_OVER_X) {
			middle::Id copyId = copyOfInsertItem(gameState);
			action = std::make_shared<bubbleActions::InsertAsXOverX>(intersectingId, copyId, gameState->mouseState.mouseXZ_PlanePos);
		}
		else if (actionType == bubbleInventoryItemType::INSERT_X_MINUS_X) {
			middle::Id copyId = copyOfInsertItem(gameState);
			action = std::make_shared<bubbleActions::InsertAsXMinusX>(intersectingId, copyId, gameState->mouseState.mouseXZ_PlanePos);
		}
	...
	void update(middle::GameState* gameState) override {
		...
		auto& inp = gameState->middleInputState.gameInput;
		...
		if (hotKeyPressed) {
			...
			for (int i = 0; i < intersectingCache->getSize(); ++i) {
				...
				if (intersecting->intersectingTop) {
					...
					// insert
					if (gameState->middleInputState.gameInput.insertTerm) {
						if (inventoryCache->relevantIdVector.size() == 1) {
							middle::Id invId = inventoryCache->relevantIdVector[0];
							auto inv = middle::getComp<components::Inventory>(gameState, invId);
							auto loop = middle::getComp<components::LoopSociety>(gameState, invId);
					...
					if (gameState->middleInputState.gameInput.pop) {
						if (!gameState->middleInputState.gameInput.shiftHeld) {
					...
					if (gameState->middleInputState.gameInput.comp) {
						if (!gameState->middleInputState.gameInput.shiftHeld) {
					...
					if (gameState->middleInputState.gameInput.mulOne) {
						if (!gameState->middleInputState.gameInput.shiftHeld) {
					...
					if (gameState->middleInputState.gameInput.can) {
						if (!gameState->middleInputState.gameInput.shiftHeld) {
					...
					if (gameState->middleInputState.gameInput.proc) {
