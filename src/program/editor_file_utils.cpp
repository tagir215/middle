...
namespace middle {
	...
	void flushBuffer(GameState* gameState, std::vector<std::string>& buffer, int type, int index) {
		...
		if (type == (int)ShapeType::CONSTRAINT) {
			...
			constraint(index, indexA, indexB, targetDistance);
			buffer.clear();
			return;
		}
